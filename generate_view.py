...
# def generate_model(con,db_dict):
#     with con:

#         f = open("model.txt", "w")

#         cur = con.cursor()
#         cur.execute("SELECT table_name FROM information_schema.tables WHERE table_schema  = '"+db_dict['schema']+"' ")
...
#             sql = "select column_name ,  data_type , udt_name , character_maximum_length , is_nullable FROM information_schema.columns WHERE table_schema = '"+ db_dict['schema'] +"' AND table_name   = '"+ table_name +"';"
#             cur.execute(sql)
#             rows = cur.fetchall()
...
#             model_code = model_code + "\n"+" "*8+"db_table = '" + db_dict['schema'] + '"."' + table_name + "'\n\n"
